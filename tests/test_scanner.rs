...
use crate::test_common::get_script_contents;
...
fn numbers() {
    ...
    let token = &token_list[0];
    assert_eq!(token.get_type(), TokenType::Number(123.00));
    assert_eq!(token.get_lexeme(), "123");
    assert_eq!(token.get_line(), 1);

    let token = &token_list[1];
    assert_eq!(token.get_type(), TokenType::Number(123.456));
    ...
    let token = &token_list[3];
    assert_eq!(token.get_type(), TokenType::Number(456.00));
...
fn strings() {
    ...
    assert_eq!(token.get_type(), TokenType::String("".to_string()));
    assert_eq!(token.get_lexeme(), "\"\"");
    assert_eq!(token.get_line(), 1);

    let token = &token_list[1];
    // Similar to the above...
    assert_eq!(token.get_type(), TokenType::String("string".to_string()));
...
fn sample() {
    ...
    let token = &token_list[8];
    assert_eq!(token.get_type(), TokenType::String("Fry until golden brown.".to_string()));
    ...
    assert_eq!(token.get_type(), TokenType::Number(5.0));
...
fn utf8_text() {
    ...
    let token = &token_list[3];
    assert_eq!(token.get_type(), TokenType::String("運去英雄飲恨多".to_string()));
    ...
    let token = &token_list[8];
    assert_eq!(token.get_type(), TokenType::String("Đông thê thê như gió thổi u hồn 😢".to_string()));
    ...
    let token = &token_list[13];
    assert_eq!(token.get_type(), TokenType::String("秋の終わり".to_string()));
